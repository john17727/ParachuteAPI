...
fun Route.areaController(service: AreaService) {
    authenticate {
        post<AreaRoute> {
            ...
            newAreaRequest.userId = call.principal<JWTPrincipal>()!!.payload.getClaim(USER_ID).asInt()
        ...
        get<AreaRoute> {
            ...
            when (val response = service.getUserAreas(userId)) {
                is Success -> call.respond(response.status, ApiResponse.success(response.data))
                is Failed -> call.respond(response.status, ApiResponse.fail(response.message))
            }
        }
        ...
        put<AreaRoute.Id> { parameters ->
            ...
            areaRequest.userId = call.principal<JWTPrincipal>()!!.payload.getClaim(USER_ID).asInt()
